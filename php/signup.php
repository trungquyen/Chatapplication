    ...
    if (!empty($fname) && !empty($lname) && !empty($email) && !empty($password)){
        ...
        if (filter_var($email, FILTER_VALIDATE_EMAIL)){
            ...
            if (mysqli_num_rows($sql) > 0){//if email already exist
                ...
                //let's check users update file or not
                if (isset($_FILES['image'])){//if file is uploaded
                    ...
                    $extensions = ['png', 'ipeg', 'jpg']; //these are some valid img ext and we've store them in array
                    if (in_array($img_ext, $extensions) === true){//is user uploaded img ext is matches with any array extensions
                        $time = time(); // this will return us current time...
                                        //we need this time because when your uploading user img to in our folder we rename user file with current time
                        ...
                        if (move_uploaded_file($tmp_name, "image/".$new_img_name)){//if user upload img move to our folder successfully
                            ...
                            if ($sql2){//if these data inseide table
